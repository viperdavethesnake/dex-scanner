...
Assumes 18 decimal places for token output (safe for most ERC20s).
    ...
    try:
        ...
        if direction == "buy":
            ...
            if amounts is None or amounts[-1] == 0:
                return None
            token_out = amounts[-1]
            price_usd = sell_usd / (token_out / 1e18) if token_out > 0 else 0.0
            ...
            price_usd = usdc_out   # total USDC received (not per-token)
        ...
        return Quote(
            source       = "aerodrome",
            price_usd    = price_usd,
            slippage_bps = 0,   # Aerodrome doesn't expose slippage bps directly
            gas_usd      = gas_usd,
